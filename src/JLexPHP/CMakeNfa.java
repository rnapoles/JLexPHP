...
class CMakeNfa {
    ...
    private void set(
        ...
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(null != input);
            JLexPHP.CUtility.ASSERT(null != lexGen);
            JLexPHP.CUtility.ASSERT(null != spec);
        }
    ...
    void allocate_BOL_EOF(
        ...
        JLexPHP.CUtility.ASSERT(JLexPHP.CSpec.NUM_PSEUDO == 2);
 ...
 /*m_spec.m_current_token = m_lexGen.EOS;
	  m_lexGen.advance();*/
        ...
        for (i = 0; i < size; ++i) {
            elem = (JLexPHP.CNfa) m_spec.m_nfa_states.elementAt(i);
            elem.m_label = i;
        }
    ...
    private JLexPHP.CNfa machine()
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("machine", m_spec.m_lexeme, m_spec.m_current_token);
        }
        ...
        JLexPHP.SparseBitSet all_states = new JLexPHP.SparseBitSet();
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("machine", m_spec.m_lexeme, m_spec.m_current_token);
        }
     ...
     * ************************************************************
     */
    private JLexPHP.CNfa rule()
            throws java.io.IOException {
        JLexPHP.CNfaPair pair;
        JLexPHP.CNfa p;
        JLexPHP.CNfa start = null;
        JLexPHP.CNfa end = null;
        int anchor = JLexPHP.CSpec.NONE;

        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("rule", m_spec.m_lexeme, m_spec.m_current_token);
        }

        pair = CAlloc.newCNfaPair();

        if (m_lexGen.AT_BOL == m_spec.m_current_token) {
            anchor = anchor | JLexPHP.CSpec.START;
            m_lexGen.advance();
            expr(pair);
        ...
        if (m_lexGen.AT_EOL == m_spec.m_current_token) {
            ...
            JLexPHP.CNfaPair nlpair = CAlloc.newNLPair(m_spec);
            ...
            anchor = anchor | JLexPHP.CSpec.END;
        }
 ...
 /* End: Removed for states. */
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("rule", m_spec.m_lexeme, m_spec.m_current_token);
        }
    ...
    private void expr(
            ...
            throws java.io.IOException {
        JLexPHP.CNfaPair e2_pair;
        JLexPHP.CNfa p;

        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("expr", m_spec.m_lexeme, m_spec.m_current_token);
        }

        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(null != pair);
        }
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("expr", m_spec.m_lexeme, m_spec.m_current_token);
        }
    }
    ...
    private void cat_expr(
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("cat_expr", m_spec.m_lexeme, m_spec.m_current_token);
        }
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("cat_expr", m_spec.m_lexeme, m_spec.m_current_token);
        }
    }
    ...
    private void factor(
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("factor", m_spec.m_lexeme, m_spec.m_current_token);
        }
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("factor", m_spec.m_lexeme, m_spec.m_current_token);
        }
    }
    ...
    private void term(
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("term", m_spec.m_lexeme, m_spec.m_current_token);
        }
        ...
        if (m_lexGen.OPEN_PAREN == m_spec.m_current_token) {
            ...
            if (false == (m_lexGen.ANY == m_spec.m_current_token
                ...
                start.m_edge = JLexPHP.CNfa.CCL;

                start.m_set = new JLexPHP.CSet();

                /* Match case-insensitive letters using character class. */
                if (m_spec.m_ignorecase && isAlphaL) {
                    start.m_set.addncase(m_spec.m_lexeme);
                } /* Match dot (.) using character class. */ else if (m_lexGen.ANY == m_spec.m_current_token) {
                    start.m_set.add('\n');
                    start.m_set.add('\r');
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("term", m_spec.m_lexeme, m_spec.m_current_token);
        }
    }
    ...
    private void dodash(
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.enter("dodash", m_spec.m_lexeme, m_spec.m_current_token);
        }
        ...
        if (JLexPHP.CUtility.DESCENT_DEBUG) {
            JLexPHP.CUtility.leave("dodash", m_spec.m_lexeme, m_spec.m_current_token);
        }
    }
}

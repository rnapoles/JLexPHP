...
class CEmit {
    ...
    private void set(
        ...
        if (JLexPHP.CUtility.DEBUG) {
            ...
            JLexPHP.CUtility.ASSERT(null != outstream);
        }
 ...
 /*m_outstream.println("\tprivate int yy_accept (int current) {");
    ...
    private void emit_table()
        ...
        for (elem = 0; elem < size; ++elem) {
            ...
            if (null != accept) {
                is_start = (0 != (m_spec.m_anchor_array[elem] & JLexPHP.CSpec.START));
                is_end = (0 != (m_spec.m_anchor_array[elem] & JLexPHP.CSpec.END));
...
//	m_outstream.print("\t\t$this->yy_rmap = $yy_rmap[0];");
        ...
        for (elem = 0; elem < size; elem++) {
            ...
            JLexPHP.CUtility.ASSERT(dtrans.m_dtrans.length == m_spec.m_dtrans_ncols);
...
//	  m_outstream.println("\t\t\t$yy_next_state = self::YY_F;");
 ...
 /*m_outstream.println("\t\t\t\t\tyy_this_accept = yy_acpt[yy_state];");
    ...
    private void emit_actions(
        ...
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(m_spec.m_accept_vector.size()
        ...
        for (elem = 0; elem < size; ++elem) {
            ...
            if (null != accept) {
                ...
                String code = new String(accept.m_action, 0, accept.m_action_read);

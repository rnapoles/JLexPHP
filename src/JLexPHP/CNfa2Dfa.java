...
class CNfa2Dfa {
 ...
 /* Create start state and initialize fields. */
            ...
            bunch.m_nfa_bit = new JLexPHP.SparseBitSet();
            ...
            bunch.m_accept = null;
            bunch.m_anchor = JLexPHP.CSpec.NONE;
            bunch.m_accept_index = JLexPHP.CUtility.INT_MAX;
            ...
            while (null != (dfa = get_unmarked())) {
                ...
                if (JLexPHP.CUtility.DEBUG) {
                    JLexPHP.CUtility.ASSERT(false == dfa.m_mark);
                }
                ...
                for (i = 0; i < m_spec.m_dtrans_ncols; ++i) {
                    if (JLexPHP.CUtility.DEBUG) {
                        JLexPHP.CUtility.ASSERT(0 <= i);
                        JLexPHP.CUtility.ASSERT(m_spec.m_dtrans_ncols > i);
                    }
                    ...
                    if (JLexPHP.CUtility.DEBUG) {
                        JLexPHP.CUtility.ASSERT((null == bunch.m_nfa_set
                    ...
                    if (JLexPHP.CUtility.DEBUG) {
                        JLexPHP.CUtility.ASSERT(nextstate < m_spec.m_dfa_states.size());
                    }
                ...
                if (JLexPHP.CUtility.DEBUG) {
                    JLexPHP.CUtility.ASSERT(m_spec.m_dtrans_vector.size() == dfa.m_label);
                }
 ...
 /* UNDONE: Don't free CAccepts. */
    ...
    private void e_closure(
        ...
        /* Debug checks. */
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(null != bunch);
            JLexPHP.CUtility.ASSERT(null != bunch.m_nfa_set);
            JLexPHP.CUtility.ASSERT(null != bunch.m_nfa_bit);
        }
        ...
        for (i = 0; i < size; ++i) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(bunch.m_nfa_bit.get(state.m_label));
            }
        ...
        while (false == nfa_stack.empty()) {
            ...
            if (null != state.m_accept
                ...
                if (JLexPHP.CUtility.DEBUG) {
                    JLexPHP.CUtility.ASSERT(null != bunch.m_accept);
                    JLexPHP.CUtility.ASSERT(JLexPHP.CSpec.NONE == bunch.m_anchor
                            || 0 != (bunch.m_anchor & JLexPHP.CSpec.END)
                            || 0 != (bunch.m_anchor & JLexPHP.CSpec.START));
                }
            }
            ...
            if (CNfa.EPSILON == state.m_edge) {
                if (null != state.m_next) {
                    if (false == bunch.m_nfa_set.contains(state.m_next)) {
                        if (JLexPHP.CUtility.DEBUG) {
                            JLexPHP.CUtility.ASSERT(false == bunch.m_nfa_bit.get(state.m_next.m_label));
                        }
                ...
                if (null != state.m_next2) {
                    if (false == bunch.m_nfa_set.contains(state.m_next2)) {
                        if (JLexPHP.CUtility.DEBUG) {
                            JLexPHP.CUtility.ASSERT(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
                        }
    ...
    private int add_to_dstates(
        ...
        if (JLexPHP.CUtility.DEBUG) {
            ...
            JLexPHP.CUtility.ASSERT(null != bunch.m_nfa_bit);
            JLexPHP.CUtility.ASSERT(null != bunch.m_accept
                    || JLexPHP.CSpec.NONE == bunch.m_anchor);
        }
        ...
        dfa.m_nfa_bit = (JLexPHP.SparseBitSet) bunch.m_nfa_bit.clone();

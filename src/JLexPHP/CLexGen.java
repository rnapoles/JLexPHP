...
class CLexGen {
    ...
    private JLexPHP.CInput m_input;
    /* Input buffer class. */
    ...
    private JLexPHP.CMakeNfa m_makeNfa;
    /* NFA machine generator module. */
    private JLexPHP.CNfa2Dfa m_nfa2dfa;
    /* NFA to DFA machine (transition table)
				 conversion module. */
    private JLexPHP.CMinimize m_minimize;
    /* Transition table compressor. */
    private JLexPHP.CSimplifyNfa m_simplifyNfa;
    /* NFA simplifier using char classes */
    private JLexPHP.CEmit m_emit;
    ...
    CLexGen(String filename) throws java.io.FileNotFoundException, java.io.IOException {
        ...
        if (index != -1) {
            ...
            name = StringUtils.ucfirst(name).concat("Lexer");
        }
        ...
        /* Open output stream. */
        m_outstream = new java.io.PrintWriter(new java.io.BufferedWriter(new java.io.FileWriter(name + ".php")));
        if (null == m_outstream) {
            System.out.println("Error: Unable to open output file " + name + ".php.");
            return;
        }

        /* Create input buffer class. */
        m_input = new JLexPHP.CInput(m_instream);
        ...
        m_spec = new JLexPHP.CSpec(this);
        if (index != -1) {
            m_spec.m_class_name = name.toCharArray();
        }
        
        /* Nfa to dfa converter. */
        m_nfa2dfa = new JLexPHP.CNfa2Dfa();
        m_minimize = new JLexPHP.CMinimize();
        m_makeNfa = new JLexPHP.CMakeNfa();
        m_simplifyNfa = new JLexPHP.CSimplifyNfa();

        m_emit = new JLexPHP.CEmit();
    ...
    void generate()
        ...
        if (false == m_init_flag) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_INIT, 0);
        }

        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(null != this);
            JLexPHP.CUtility.ASSERT(null != m_outstream);
            JLexPHP.CUtility.ASSERT(null != m_input);
            JLexPHP.CUtility.ASSERT(null != m_tokens);
            JLexPHP.CUtility.ASSERT(null != m_spec);
            JLexPHP.CUtility.ASSERT(m_init_flag);
        }
        ...
        if (m_input.m_eof_reached) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_EOF, m_input.m_line_number);
        }
        ...
        if (m_spec.m_verbose && true == JLexPHP.CUtility.OLD_DUMP_DEBUG) {
            details();
        }
    ...
    private void userCode()
        ...
        if (m_input.m_eof_reached) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_EOF, 0);
        }
    ...
    private char[] getName() {
        ...
        while (m_input.m_line_index < m_input.m_line_read
                && true == JLexPHP.CUtility.isspace(m_input.m_line[m_input.m_line_index])) {
            ++m_input.m_line_index;
        }

        /* No name? */
        if (m_input.m_line_index >= m_input.m_line_read) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_DIRECT, 0);
        }

        /* Determine length. */
        elem = m_input.m_line_index;
        while (elem < m_input.m_line_read
                && false == JLexPHP.CUtility.isnewline(m_input.m_line[elem])) {
            ++elem;
        }
        ...
        elem = 0;
        while (m_input.m_line_index < m_input.m_line_read
                && false == JLexPHP.CUtility.isnewline(m_input.m_line[m_input.m_line_index])) {
    ...
    private char[] packCode(
            ...
            throws java.io.IOException {
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(INIT_CODE == specified
        ...
        if (0 != JLexPHP.CUtility.charncmp(m_input.m_line,
                0,
                start_dir,
                0,
                start_dir.length - 1)) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_INTERNAL, 0);
        }
        ...
        if (prev_read >= prev_code.length) {
            prev_code = JLexPHP.CUtility.doubleSize(prev_code);
        }
            ...
            while (m_input.m_line_index >= m_input.m_line_read) {
                ...
                if (0 == JLexPHP.CUtility.charncmp(m_input.m_line,
                    ...
                    switch (specified) {
                        ...
                        default:
                            JLexPHP.CError.parse_error(JLexPHP.CError.E_INTERNAL, m_input.m_line_number);
                            break;
                    }
    ...
    private void userDeclare()
        ...
        if (m_input.m_eof_reached) {
            /* End-of-file. */
            JLexPHP.CError.parse_error(JLexPHP.CError.E_EOF,
        ...
        while (false == m_input.getLine()) {
            ...
            if ('%' == m_input.m_line[0]) {
                ...
                if (1 >= m_input.m_line_read) {
                    JLexPHP.CError.parse_error(JLexPHP.CError.E_DIRECT,
                            m_input.m_line_number);
                    continue;
                }
                ...
                switch (m_input.m_line[1]) {
                    ...
                    case 'c':
                        if (0 == JLexPHP.CUtility.charncmp(m_input.m_line,
                            ...
                            break;
                        } else if (0 == JLexPHP.CUtility.charncmp(m_input.m_line,
                                0,
                                m_class_dir,
                                0,
                    ...
                    case 'f':
                        if (0 == JLexPHP.CUtility.charncmp(m_input.m_line,
                            ...
                            m_spec.m_dtrans_ncols = JLexPHP.CUtility.MAX_EIGHT_BIT + 1;
                            break;
                        }
                    ...
                    case 'u':
                        if (0 == JLexPHP.CUtility.charncmp(m_input.m_line,
                            ...
                            m_spec.m_dtrans_ncols = JLexPHP.CUtility.MAX_SIXTEEN_BIT + 1;
                            break;
                        }
    ...
    private void userRules()
        ...
        /*print_nfa();*/
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(END_OF_INPUT == m_spec.m_current_token);
        }
    ...
    void print_nfa() {
        ...
        for (elem = 0; elem < size; ++elem) {
            nfa = (JLexPHP.CNfa) m_spec.m_nfa_states.elementAt(elem);
            ...
            if (null == nfa.m_next) {
                ...
                switch (nfa.m_edge) {
                    case JLexPHP.CNfa.CCL:
                        printccl(nfa.m_set);
                        break;

                    case JLexPHP.CNfa.EPSILON:
            ...
            if (null != nfa.m_accept) {
                System.out.print(" accepting "
                        + ((0 != (nfa.m_anchor & JLexPHP.CSpec.START)) ? "^" : "")
                        + "<"
                        + (new String(nfa.m_accept.m_action, 0,
                                nfa.m_accept.m_action_read))
                        + ">"
                        + ((0 != (nfa.m_anchor & JLexPHP.CSpec.END)) ? "$" : ""));
            }
        ...
        while (states.hasMoreElements()) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(null != state);
                JLexPHP.CUtility.ASSERT(null != index);
            }
            ...
            for (j = 0; j < vsize; ++j) {
                nfa = (JLexPHP.CNfa) m_spec.m_state_rules[i].elementAt(j);
     ...
     * < state1, state2 ... > regular_expression { action } Returns null on only
     * EOF. Returns all_states, initialied properly to correspond to all states,
     * if no states are found. Special Notes: This function treats commas as
     * optional and permits states to be spread over multiple lines.
     * ************************************************************
     */
    private JLexPHP.SparseBitSet all_states = null;

    JLexPHP.SparseBitSet getStates()
            throws java.io.IOException {
        int start_state;
        int count_state;
        JLexPHP.SparseBitSet states;
        String name;
        Integer index;
        int i;
        int size;
        ...
        /* Skip white space. */
        while (JLexPHP.CUtility.isspace(m_input.m_line[m_input.m_line_index])) {
            ++m_input.m_line_index;
        ...
        if ('<' == m_input.m_line[m_input.m_line_index]) {
            ...
            states = new JLexPHP.SparseBitSet();
            ...
            while (true) {
                ...
                while (false == JLexPHP.CUtility.isspace(m_input.m_line[m_input.m_line_index])
                ...
                if (null == index) {
                    ...
                    JLexPHP.CError.parse_error(JLexPHP.CError.E_STATE, m_input.m_line_number);
                }
        ...
        if (null == all_states) {
            all_states = new JLexPHP.SparseBitSet();
    ...
    private boolean expandMacro() {
        ...
        if (elem >= m_input.m_line_read) {
            JLexPHP.CError.impos("Unfinished macro name");
            return ERROR;
        }
        ...
        while ('}' != m_input.m_line[elem]) {
            ++elem;
            if (elem >= m_input.m_line_read) {
                JLexPHP.CError.impos("Unfinished macro name at line "
                        + m_input.m_line_number);
                return ERROR;
            }
        }
        ...
        /* Check macro name. */
        if (0 == count_name) {
            JLexPHP.CError.impos("Nonexistent macro name");
            return ERROR;
        }

        /* Debug checks. */
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(0 < count_name);
        }
        ...
        if (null == def) {
            ...
            JLexPHP.CError.parse_error(JLexPHP.CError.E_NOMAC, m_input.m_line_number);
            return ERROR;
        }
        if (JLexPHP.CUtility.OLD_DUMP_DEBUG) {
	   ...
	   beginning by copying first part of line buffer. */
        ...
        for (rep_elem = 0; rep_elem < start_macro; ++rep_elem) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(rep_elem < replace.length);
            }
        }

        /* Copy macro definition. */
        if (rep_elem >= replace.length) {
            replace = JLexPHP.CUtility.doubleSize(replace);
        }
    ...
    private void saveMacro() {
	   ...
	   macro_name macro_definition */
        ...
        while (JLexPHP.CUtility.isspace(m_input.m_line[elem])) {
            ++elem;
            if (elem >= m_input.m_line_read) {
        ...
        /* Read macro name. */
        start_name = elem;
        while (false == JLexPHP.CUtility.isspace(m_input.m_line[elem])
                && '=' != m_input.m_line[elem]) {
            ++elem;
            if (elem >= m_input.m_line_read) {
                /* Macro name but no associated definition. */
                JLexPHP.CError.parse_error(JLexPHP.CError.E_MACDEF, m_input.m_line_number);
            }
        }
        count_name = elem - start_name;
        ...
        /* Debug checks. */
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(0 < count_def);
            JLexPHP.CUtility.ASSERT(0 < count_name);
            JLexPHP.CUtility.ASSERT(null != m_spec.m_macros);
        }
    ...
    private void saveStates() {
        ...
        /* Debug checks. */
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT('%' == m_input.m_line[0]);
            JLexPHP.CUtility.ASSERT('s' == m_input.m_line[1]);
            JLexPHP.CUtility.ASSERT(m_input.m_line_index <= m_input.m_line_read);
            JLexPHP.CUtility.ASSERT(0 <= m_input.m_line_index);
            JLexPHP.CUtility.ASSERT(0 <= m_input.m_line_read);
        }
        ...
        if ('\\' != m_input.m_line[m_input.m_line_index]) {
            ...
            switch (m_input.m_line[m_input.m_line_index]) {
                ...
                case '^':
                    ...
                    if (r < '@' || r > 'Z') // non-fatal
                    {
                        JLexPHP.CError.parse_error(JLexPHP.CError.E_BADCTRL, m_input.m_line_number);
                    }
                    r = (char) (r - '@');
                    ++m_input.m_line_index;
                    return r;
                ...
                case 'x':
                    ...
                    for (int i = 0; i < (unicode_escape ? 4 : 2); i++) {
                        if (JLexPHP.CUtility.ishexdigit(m_input.m_line[m_input.m_line_index])) {
                            r = (char) (r << 4);
                            r = (char) (r | JLexPHP.CUtility.hex2bin(m_input.m_line[m_input.m_line_index]));
                            ++m_input.m_line_index;
                        } else {
                            break;
                        }
                ...
                default:
                    if (false == JLexPHP.CUtility.isoctdigit(m_input.m_line[m_input.m_line_index])) {
                        r = m_input.m_line[m_input.m_line_index];
                        ++m_input.m_line_index;
                    } else {
                        r = 0;
                        for (int i = 0; i < 3; i++) {
                            if (JLexPHP.CUtility.isoctdigit(m_input.m_line[m_input.m_line_index])) {
                                r = (char) (r << 3);
                                r = (char) (r | JLexPHP.CUtility.oct2bin(m_input.m_line[m_input.m_line_index]));
                                ++m_input.m_line_index;
                            } else {
                                break;
                            }
     ...
     * *****************************************************
     */
    JLexPHP.CAccept packAccept()
            throws java.io.IOException {
        JLexPHP.CAccept accept;
        char action[];
        int action_index;
        int brackets;
        ...
        /* Look for brackets. */
        if ('{' != m_input.m_line[m_input.m_line_index]) {
            JLexPHP.CError.parse_error(JLexPHP.CError.E_BRACE, m_input.m_line_number);
        }
        ...
        while (true) {
            ...
            if (action_index >= action.length) {
                action = JLexPHP.CUtility.doubleSize(action);
            }
            ...
            while (m_input.m_line_index >= m_input.m_line_read) {
                ...
                if (insinglequotes || indoublequotes) { // non-fatal
                    JLexPHP.CError.parse_error(JLexPHP.CError.E_NEWLINE, m_input.m_line_number);
                    insinglequotes = indoublequotes = false;
                }
                if (m_input.getLine()) {
                    JLexPHP.CError.parse_error(JLexPHP.CError.E_SYNTAX, m_input.m_line_number);
                    return null;
                }
            }
        }

        accept = new JLexPHP.CAccept(action, action_index, m_input.m_line_number);

        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(null != accept);
        }

        if (JLexPHP.CUtility.DESCENT_DEBUG) {
	  ...
	  CUtility.ASSERT(m_input.m_line_index <= m_input.m_line_read);
	   ...
	   Refill line buffer? */
        if (EOS == m_spec.m_current_token
                /* ADDED */
                || m_input.m_line_index >= m_input.m_line_read) /* ADDED */ {
	   ...
	   escape sequence. */
        ...
        if (false == m_spec.m_in_quote) {
            if (false == m_spec.m_in_ccl
                    && JLexPHP.CUtility.isspace(m_input.m_line[m_input.m_line_index])) {
    ...
    private void details() {
        ...
        while (names.hasMoreElements()) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(null != name);
                JLexPHP.CUtility.ASSERT(null != def);
            }
        ...
        if (false == m_spec.m_count_chars) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(m_spec.m_count_lines);
            }
    ...
    void print_set(
        ...
        for (elem = 0; elem < size; ++elem) {
            nfa = (JLexPHP.CNfa) nfa_set.elementAt(elem);
            /*System.out.print(m_spec.m_nfa_states.indexOf(nfa) + " ");*/
    ...
    private void print_header() {
        ...
        while (states.hasMoreElements()) {
            ...
            i = index.intValue();
            if (JLexPHP.CDTrans.F != m_spec.m_state_dtrans[i]) {
        ...
        for (i = 0; i < m_spec.m_dtrans_vector.size(); ++i) {
            dtrans = (JLexPHP.CDTrans) m_spec.m_dtrans_vector.elementAt(i);
            ...
            if (null == m_spec.m_accept_vector && null == m_spec.m_anchor_array) {
                if (null == dtrans.m_accept) {
                    ...
                    System.out.print(" * State " + i
                            ...
                            + ">]");
                    if (JLexPHP.CSpec.NONE != dtrans.m_anchor) {
                        System.out.print(" Anchor: "
                                + ((0 != (dtrans.m_anchor & JLexPHP.CSpec.START))
                                        ? "start " : "")
                                + ((0 != (dtrans.m_anchor & JLexPHP.CSpec.END))
                                        ? "end " : ""));
                    }
                }
            } else {
                accept = (JLexPHP.CAccept) m_spec.m_accept_vector.elementAt(i);
                ...
                if (null == accept) {
                    ...
                    System.out.print(" * State " + i
                            ...
                            + ">]");
                    if (JLexPHP.CSpec.NONE != m_spec.m_anchor_array[i]) {
                        System.out.print(" Anchor: "
                                + ((0 != (m_spec.m_anchor_array[i] & JLexPHP.CSpec.START))
                                        ? "start " : "")
                                + ((0 != (m_spec.m_anchor_array[i] & JLexPHP.CSpec.END))
                                        ? "end " : ""));
                    }
                }
            }
            ...
            for (j = 0; j < m_spec.m_dtrans_ncols; ++j) {
                if (JLexPHP.CDTrans.F != dtrans.m_dtrans[j]) {

...
class CMinimize {
    ...
    private void reduce() {
        ...
        int reduced_ncols;
        int reduced_nrows;
        JLexPHP.SparseBitSet set;
        CDTrans dtrans;
        int size;

        set = new JLexPHP.SparseBitSet();
        ...
        for (reduced_ncols = 0;; ++reduced_ncols) {
            if (JLexPHP.CUtility.DEBUG) {
                for (i = 0; i < reduced_ncols; ++i) {
                    JLexPHP.CUtility.ASSERT(-1 != m_spec.m_col_map[i]);
                }
            }
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(false == set.get(i));
                JLexPHP.CUtility.ASSERT(-1 == m_spec.m_col_map[i]);
            }
        ...
        if (JLexPHP.CUtility.DEBUG) {
            JLexPHP.CUtility.ASSERT(k == reduced_ncols);
        }
        ...
        for (reduced_nrows = 0;; ++reduced_nrows) {
            if (JLexPHP.CUtility.DEBUG) {
                for (i = 0; i < reduced_nrows; ++i) {
                    JLexPHP.CUtility.ASSERT(-1 != m_spec.m_row_map[i]);
                }
            }
        ...
        if (JLexPHP.CUtility.DEBUG) {
            ...
            JLexPHP.CUtility.ASSERT(k == reduced_nrows);
        }
    }
    ...
    private void minimize() {
        ...
        while (old_group_count != group_count) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(m_group.size() == group_count);
            }
                ...
                for (j = 1; j < group_size; ++j) {
                    ...
                    for (c = 0; c < m_spec.m_dtrans_ncols; ++c) {
                        ...
                        if (goto_first != goto_next
                            ...
                            if (JLexPHP.CUtility.DEBUG) {
                                JLexPHP.CUtility.ASSERT(dtrans_group.elementAt(j) == next);
                            }
                            ...
                            if (JLexPHP.CUtility.DEBUG) {
                                JLexPHP.CUtility.ASSERT(m_group.contains(new_group)
                                        == true);
                                JLexPHP.CUtility.ASSERT(m_group.contains(dtrans_group)
                                        == true);
                                JLexPHP.CUtility.ASSERT(dtrans_group.contains(first)
                                        == true);
                                JLexPHP.CUtility.ASSERT(dtrans_group.contains(next)
                                        == false);
                                JLexPHP.CUtility.ASSERT(new_group.contains(first)
                                        == false);
                                JLexPHP.CUtility.ASSERT(new_group.contains(next)
                                        == true);
                                JLexPHP.CUtility.ASSERT(dtrans_group.size() == group_size);
                                JLexPHP.CUtility.ASSERT(i == m_ingroup[first.m_label]);
                                JLexPHP.CUtility.ASSERT((m_group.size() - 1)
        ...
        if (m_spec.m_verbose
                && true == JLexPHP.CUtility.OLD_DUMP_DEBUG) {
            System.out.println();
    ...
    private void init_groups() {
        ...
        for (i = 0; i < size; ++i) {
            ...
            if (JLexPHP.CUtility.DEBUG) {
                JLexPHP.CUtility.ASSERT(i == dtrans.m_label);
                JLexPHP.CUtility.ASSERT(false == group_found);
                JLexPHP.CUtility.ASSERT(group_count == m_group.size());
            }
            ...
            for (j = 0; j < group_count; ++j) {
                ...
                if (JLexPHP.CUtility.DEBUG) {
                    JLexPHP.CUtility.ASSERT(false == group_found);
                    JLexPHP.CUtility.ASSERT(0 < dtrans_group.size());
                }

                first = (CDTrans) dtrans_group.elementAt(0);

                if (JLexPHP.CUtility.SLOW_DEBUG) {
                    CDTrans check;
                    int k;
                    int s;

                    s = dtrans_group.size();
                    JLexPHP.CUtility.ASSERT(0 < s);

                    for (k = 1; k < s; ++k) {
                        check = (CDTrans) dtrans_group.elementAt(k);
                        JLexPHP.CUtility.ASSERT(check.m_accept == first.m_accept);
                    }
                }
                ...
                if (first.m_accept == dtrans.m_accept) {
                    ...
                    if (JLexPHP.CUtility.DEBUG) {
                        JLexPHP.CUtility.ASSERT(j == m_ingroup[dtrans.m_label]);
                    }
